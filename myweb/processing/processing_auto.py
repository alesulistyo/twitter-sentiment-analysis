...
df = pd.read_csv('hasil_predict.csv', encoding='unicode_escape')
...
filename1 = 'count_vectorized1_auto.pkl'
...
filename1 = 'tfid_transform1_auto.pkl'
...
with open('classification_auto.csv', 'w') as f:
    for line in s:
        f.write(line)

accuracy = accuracy_score(y_test, pred)
a = np.asarray([accuracy])
np.savetxt("accuracy_auto.csv", a, comments='',
           delimiter=",", fmt='%.2f', header='accuracy')

precision = precision_score(y_test, pred, average='macro')
p = np.asarray([precision])
np.savetxt("precision_auto.csv", p, comments='',
           delimiter=",", fmt='%.2f', header='precision')

recall = recall_score(y_test, pred, average='macro')
r = np.asarray([recall])
np.savetxt("recall_auto.csv", r, comments='',
           delimiter=",", fmt='%.2f', header='recall')

f1 = f1_score(y_test, pred, average='macro')
f = np.asarray([f1])
np.savetxt("f1_auto.csv", f, comments='',
           delimiter=",", fmt='%.2f', header='f1')

final = df[['tweets', 'label', 'polarity']]
final.to_csv('hasil_processing_auto.csv', encoding='utf-8', index=False)

model = LogisticRegression()
model.fit(X_train, y_train)

filename = 'model_analisis_auto.pkl'
